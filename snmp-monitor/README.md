...
- **Trap Receiver:** Un demone asincrono costantemente in ascolto sulla porta UDP. Il suo compito è intercettare le notifiche asincrone (_Trap_) inviate dagli agenti a fronte di anomalie o variazioni di stato (es. un'interfaccia che va in _down_).
...
- **RRD Manager & Data Processor:** Un modulo che si occupa della persistenza dei dati. Memorizza le metriche all'interno di un file strutturato metrics.csv e, alla chiusura del programma, sfrutta la libreria **RRDtool** (Round Robin Database) per elaborare i dati storici e generare grafici temporali in formato .png.
...
- **Reazione Event-Driven:** Parallelamente al ciclo ordinario, il _Trap Receiver_ rimane in ascolto di messaggi asincroni provenienti dalla rete. Non appena viene intercettata una notifica di anomalia (_Trap_), il thread principale interrompe l'attesa del timer e **forza un ciclo di polling straordinario immediato** verso l'agente che ha generato la segnalazione, registrando all'istante lo stato esatto dei contatori.
...
**Nota per l'ambiente Windows:** Per evitare complessità legate alla compilazione dei binding C della libreria rrdtool nativa su Windows, si consiglia di utilizzare il terminale di VS Code connesso a una distribuzione Linux tramite l'estensione **WSL (Windows Subsystem for Linux)** di VS Code, oppure procedere al deployment tramite Docker.

### 2.4 Configurazione dell'Ambiente Virtuale in VS Code (Comune a tutti i S.O.)

Una volta installati i tool di sistema, aprire la cartella del progetto in VS Code (File > Open Folder...). Aprire il terminale integrato e digitare i comandi per creare e attivare l'ambiente virtuale locale prima di installare i pacchetti Python:
...
_Nota: Assicurarsi che il file config/agents.yml sia configurato per puntare a 127.0.0.1 sulle rispettive porte 1161 e 1163._
...
2026-07-06 18:32:46,014 INFO \__main_\_: Trap receiver avviato su udp://127.0.0.1:9162

\[TRAP\] Receiver avviato su udp://127.0.0.1:9162 community=public

\[TRAP\] In attesa di trap SNMP...

2026-07-06 18:32:46,130 INFO \__main_\_: Polling completato: 22 metriche raccolte da 2 agenti

2026-07-06 18:32:56,065 INFO \__main_\_: Polling completato: 22 metriche raccolte da 2 agenti

2026-07-06 18:33:06,063 INFO \__main_\_: Polling completato: 22 metriche raccolte da 2 agenti

### Test 2: Verifica del Trap-Directed Polling (Reattività ad Anomalia)

- **Procedura:** Mentre il manager è in esecuzione nel Terminale 1, spostarsi nel **Terminale 2** ed eseguire il comando nativo per inviare una notifica asincrona di guasto interfaccia (linkDown contrassegnata dall'OID standard 1.3.6.1.6.3.1.1.5.3):

snmptrap -v 2c -c public localhost:9162 "" 1.3.6.1.6.3.1.1.5.3

- **Risultato nei Log (Terminale 1):** Il Trap Receiver intercetta immediatamente il pacchetto UDP sulla porta 9162. Il flusso principale interrompe l'attesa del timer standard e avvia un'operazione di polling immediata per registrare istantaneamente lo stato del dispositivo:

\[TRAP RICEVUTA\] Notifica asincrona da 127.0.0.1 - OID: 1.3.6.1.6.3.1.1.5.3

2026-07-06 18:33:11,421 INFO \__main_\_: Forzatura Trap-Directed Polling per l'agente associato

2026-07-06 18:33:11,850 INFO snmp_monitor.poller: \[snmp-agent-1\] Walk straordinaria completata con successo
...
^C2026-07-06 18:33:20,105 INFO \__main_\_: Segnale di stop ricevuto, chiusura in corso...
